...
import { deleteMajors } from '@/api/MajorAPI'

interface Props {
  data: MajorsType
}

const MajorDelete = ({ data }: Props) => {
  const queryClient = useQueryClient()

  const deleteMutation = useMutation({
    mutationFn: async () => await deleteMajors(Number(data.id)),
    onSuccess: () => {
  ...
  return (
    <AlertDialog.Root>
      ...
      <DeleteDialog
        title="ព្រមាន"
        description={`តើអ្នកពិតជាចង់លុបមុខជំនាញ${data.name} មែនទេ?`}
